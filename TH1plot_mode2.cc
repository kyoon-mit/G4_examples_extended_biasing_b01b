...
// Use: root -l TH1plot_mode2.cc'(input, output, TTree directory)'
...
void TH1plot_mode2 (const char *input_file = "B01b.root",
              const char *output_file = "TH1_mode2.root",
...
{  
  ...
  {
    // Create histogram
    std::array<TH1F*, GLOBAL_NCELLS> hist = hist_layer(t, num_events, branchname, "", "# of secondaries", 30, 0, 30);
    return hist;
  }
  ...
  {   
    // Create histogram
    std::array<TH1F*, GLOBAL_NCELLS> hist = hist_layer(t, num_events, branchname, "", "energy deposit [MeV]", 30, 0, 60);
    return hist; 
  }
  ...
  {
    // Create histogram
    std::array<TH1F*, GLOBAL_NCELLS> hist = hist_layer(t, num_events, branchname, "", "# tracks entering", 10, 0, 10);
    return hist;
  }
  ...
  {
    // Create histogram
    std::array<TH1F*, GLOBAL_NCELLS> hist = hist_layer(t, num_events, branchname, "", "# tracks exiting", 10, 0, 10);
    return hist;
  }
  ...
  {
    // Create histogram
    std::array<TH1F*, GLOBAL_NCELLS> hist = hist_layer(t, num_events, branchname, "", "# hits", 20, 0, 200);
    return hist;
  }
  ...
  //     if (weighted) br_prestepweight->GetEntry(row);
    ...
    for (auto i = 0; i < vec_layernum->size(); ++i)
    {
      ...
      if (!output[cell])
      {
        ...
        hist->SetCanExtend(TH1::kAllAxes);
        hist->SetTitle(Form("Cell %d: %s", cell, branchname));
